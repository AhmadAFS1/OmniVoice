...
def load_model_for_export(model_path: str) -> OmniVoice:
    logging.info("Loading OmniVoice from %s on CPU for Core ML export ...", model_path)
...
def export_coreml_backbone(
    ...
    *,
    ...
    model = load_model_for_export(model_path)
    ...
    mlmodel = ct.convert(
        ...
        compute_precision=_resolve_precision(ct, precision),
        minimum_deployment_target=_resolve_target(ct, target),
    )
    ...
    metadata = {
        ...
        "fixed_seq_len": None if dynamic_seq else seq_len,
        "max_seq_len": seq_len,
        "dynamic_seq": bool(dynamic_seq),
